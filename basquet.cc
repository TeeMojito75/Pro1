...
int scoring_ability(const vector<Game>& v, const string& team) {
     ...
     for (int i = 0; i < v.size(); ++i) {
         if (team == v.home or team == v.visitor) {
             if (team == v.home) punts_casa += v.shome;
             if (team == v.visitor) punts_visit += v.svisitor;
        }
    }
...
int main() {
    int n;
    cin >> n;
    vector<Game> llista_partits(n) = read_games(n);
    vector<string> equips;
    string s;
    while (cin >> s) {
        equips.push_back(equips);
    }
